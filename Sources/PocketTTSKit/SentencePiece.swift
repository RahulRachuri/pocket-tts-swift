...
///     Mirrors `sentencepiece`'s `Model::Encode`: at each codepoint boundary all trie
...
public struct SentencePieceModel {
    public enum PieceType: Int { case normal = 1, unknown = 2, control = 3, userDefined = 4, unused = 5, byte = 6 }
    ...
    public init(url: URL) throws {
        ...
        let root = try ProtoReader(Array(data))
    ...
    public func normalize(_ text: String) -> [UInt8] {
        var t = text
...
final class ProtoReader {
    ...
    private func varint() throws -> UInt64 {
        ...
        while true {
            guard i < buf.endIndex else { throw TTSError.message("protobuf: truncated varint") }
            let b = buf[i]; i += 1
            r |= UInt64(b & 0x7F) << s
    ...
    func next() throws -> Field? {
        ...
        case 1, 5:
            let len = wire == 1 ? 8 : 4
            guard i + len <= buf.endIndex else { throw TTSError.message("protobuf: truncated fixed") }
            defer { i += len }
            return Field(number: number, wire: wire, intValue: 0, byteValue: buf[i..<(i + len)])
        case 2:
            let len = Int(try varint())
            guard i + len <= buf.endIndex else { throw TTSError.message("protobuf: truncated bytes") }
            defer { i += len }
            return Field(number: number, wire: wire, intValue: 0, byteValue: buf[i..<(i + len)])
        default:
            throw TTSError.message("protobuf: unsupported wire type \(wire)")
        }
    }
}
