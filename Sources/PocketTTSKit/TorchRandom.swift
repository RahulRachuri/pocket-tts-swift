...
public struct TorchRNG {
    ...
    public mutating func normalFill(count: Int, std: Float) -> [Float] {
...
public struct NoiseSource {
    ...
    public init(seed: UInt64, temp: Float) {
        self.seed = seed
        self.std = Float(Double(temp).squareRoot())
    }

    public mutating func nextNoise(count: Int) -> [Float] {
        calls += 1
        var rng = TorchRNG(seed: seed &+ UInt64(calls))
